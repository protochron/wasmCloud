...
#[tokio::test]
async fn test_kvstore_basic() -> anyhow::Result<()> {
...
#[tokio::test]
async fn test_kvstore_put_secret() -> anyhow::Result<()> {
    let client = async_nats::connect("127.0.0.1:4222").await?;
    ...
    let decrypted = request_key.open(&resp.payload, &server_xkey)?;
...
#[tokio::test]
async fn test_kvstore_version() -> anyhow::Result<()> {
    let client = async_nats::connect("127.0.0.1:4222").await?;
