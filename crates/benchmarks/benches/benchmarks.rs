...
"#;
    ...
    let resp = ctl_client
        ...
        .await
        .map_err(|e| anyhow::anyhow!(e).context("failed to get host inventory"))?;
    let data = resp.data().unwrap();
    println!("Host inventory: {:?}", data);
...
pub async fn run_benchmark(client: &reqwest::Client, http_port: u16, num_requests: u64) {
    let mut js = JoinSet::new();
    for _ in 0..num_requests {
        let jh = client.get(format!("http://localhost:{}", http_port)).send();
        js.spawn(jh);
    }
...
pub fn host_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("benchmark", |b| {
        b.to_async(&runtime)
            .iter(|| run_benchmark(&client, port, size));
    });
    group.finish();
    runtime.block_on(container.stop());
}
